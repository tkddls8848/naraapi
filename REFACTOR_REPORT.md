...
| tailwindcss | **3.4.19** | 이 리팩토링 시점 기준. 이후 화면 리디자인과 함께 v4 로 올렸다 — 6장 판단 2번 참조 |
| mongoose | 8.24.2 | |
...
2. **Tailwind v4 대신 v3.4.19 유지** → *후속 작업에서 v4 로 전환*
   레거시 마크업이 `shadow`(93회), `rounded`(55회), `outline-none`(29회)을 쓰는데
   v4에서 이 세 유틸리티의 의미가 바뀐다. "화면 모양을 바꾸지 않는다"는 원칙상 v3가 맞았다.
   이후 사용자가 화면 현대화와 버전 변경을 허용해, 문제가 됐던 유틸리티를 쓰는 마크업 자체를
   `@utility` 기반 공용 클래스로 교체하면서 **tailwindcss 4.3.3** 으로 올렸다
   (`tailwind.config.js` 제거, `@tailwindcss/postcss` 단일 플러그인, autoprefixer 제거).
